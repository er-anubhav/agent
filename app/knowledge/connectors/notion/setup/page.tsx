...
import { NotionTokenManager } from "@/lib/notion/tokenManager";
...
export default function NotionSetupPage() {
  ...
  useEffect(() => {
    ...
    const existingToken = NotionTokenManager.getAccessToken();
    if (existingToken) {
      setAccessToken(existingToken);
      setIsConnected(true);
      setStep(2);
      // Load pages with existing token
      loadNotionPages(existingToken);
      return;
    }
  ...
  const handleOAuthCallback = async (code: string, state: string | null) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        NotionTokenManager.setTokens(data.accessToken);
  ...
  const handleDisconnect = () => {
    if (confirm('Are you sure you want to disconnect Notion? This will clear your stored tokens.')) {
      NotionTokenManager.clearTokens();
                  ...
                  pages.map((page) => (
                    <div key={page.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50">
                      ...
                      <div className="flex items-center gap-3 text-sm text-muted-foreground">
                        <span>
                          Last modified: {new Date(page.lastModified).toISOString().split('T')[0]}
                        </span>
