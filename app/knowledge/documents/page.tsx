...
export default function DocumentsPage() {
  ...
  const LinkIcon = ({ size = 16 }: { size?: number }) => (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="currentColor">
      <path d="M4.715 6.542 3.248 7.966a3 3 0 1 0 4.243 4.243l1.424-1.424a3 3 0 0 0 .814-3.114l-.75.75a1.5 1.5 0 0 1-.404 1.06l-1.424 1.424a1.5 1.5 0 0 1-2.122-2.122l.75-.75a3 3 0 0 0-1.114-.814ZM9.293 2.293a3 3 0 0 1 4.243 0 3 3 0 0 1 0 4.243l-1.424 1.424a3 3 0 0 1-3.114.814l.75-.75a1.5 1.5 0 0 0 1.06-.404l1.424-1.424a1.5 1.5 0 0 0-2.122-2.122l-.75.75a3 3 0 0 1-.814-1.114Z"/>
    </svg>
  );

  const getSourceIcon = (source?: string) => {
    switch (source) {
      case 'google-drive': return <div className="w-4 h-4 rounded bg-blue-500 flex items-center justify-center text-white text-xs font-bold">GD</div>;
      case 'notion': return <LinkIcon size={16} />;
      case 'url': return <LinkIcon size={16} />;
      default: return <UploadIcon size={16} />;
    }
  };
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`text-sm ${getStatusColor(document.status)}`}>
                              {typeof document.status === 'string' && document.status.length > 0
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                          <span>Type: {typeof document.type === 'string' && document.type.length > 0 ? document.type.toUpperCase() : ''}</span>
                          <span>Size: {
                          ...
                          <span>Uploaded: {
                            document.uploadedAt && !isNaN(new Date(document.uploadedAt).getTime())
                              ? new Date(document.uploadedAt).toISOString().split('T')[0]
                              : 'Unknown'
                          }</span>
                          {document.sourceMetadata?.webViewLink && (
                            <a 
                              href={document.sourceMetadata.webViewLink} 
                              target="_blank" 
